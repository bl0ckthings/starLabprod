...
import swapImage from './img/swap.png';
import settingsImage from './img/settings.png';
import App from './App';
import layer0Chain from './utils/layer0ChainsInfos.json'
import chains from './utils/chains.json'
import abi from './utils/abi.json';
import endpointAbi from './utils/endointABI.json';
import compatibilityMap from './utils/chainsCompatiblity.json';
...
const Layer0Transaction = styled.div`
...
const BridgePage = ({ ethereumProvider, switchChain, account }) => {
  const defaultSrcChain = layer0Chain.find(chain => chain.chainName === 'zkSyncEraMainnet');
  const defaultDestinationChain = layer0Chain.find(chain => chain.chainName === 'binanceSmartChain');
  const [provider, setProvider] = useState(null);
  const [signer, setSigner] = useState(null);
  const [showInput, setShowInput] = useState(false);
  const [srcChainInfo, setSrcChainInfo] = useState(defaultSrcChain);
  const [destinationChainInfo, setDestinationChainInfo] = useState(defaultDestinationChain);
  const [tokenId, setTokenId] = useState('');
  const [txHash, setTxHash] = useState('');
  const [incompatibleChains, setIncompatibleChains] = useState([]);
  const [isOnRightNetwork, setIsOnRightNetwork] = useState(true);
  ...
  // Swap destination chain into source Chain
  const swapChains = () => {
    const temp = destinationChainInfo;


    // Switch chains for both srcChain and destinationChain
    switchChain(ethereumProvider, chains[destinationChainInfo.chainName]);
    switchChain(ethereumProvider, chains[temp.chainName]);
    setSrcChainInfo(temp);
    setDestinationChainInfo(srcChainInfo);
    console.log(temp);
  }


  useEffect(() => {
    setSrcChainInfo(defaultSrcChain || {});
  }, [defaultSrcChain]);
  ...
  // HANDLE PROVIDER & SIGNER
  useEffect(() => {

    const getProvider = async () => {
      const ethereumProvider = await detectEthereumProvider();
      if (ethereumProvider) {
        const provider = new Web3Provider(ethereumProvider, "any");
  ...
  // HANDLE SOURCE AND DESTINATION CHAIN INFOS ---- ALSO HANDLE THE SWITCH OF CHAINS(Only For SrcChain)
  useEffect(() => {
    const compatibleChains = compatibilityMap[srcChainInfo.chainName] || [];

    // Get the chains that are not compatible
    const initialIncompatibleChains = layer0Chain
      .map(chain => chain.chainName)
      .filter(chainName => !compatibleChains.includes(chainName));

    setIncompatibleChains(initialIncompatibleChains);
  }, [srcChainInfo]);


  const handleSourceChainChange = (event) => {
    const selectedChainName = event.target.dataset.value;
    const selectedChain = layer0Chain.find(chain => chain.chainName === selectedChainName);

    if (selectedChain && selectedChain.chainId !== srcChainInfo.chainId) {
      switchChain(ethereumProvider, chains[selectedChain.chainName]);
    }

    setSrcChainInfo(selectedChain);

    const compatibleChains = compatibilityMap[selectedChainName] || [];

    if (compatibleChains.length > 0) {
      const firstCompatibleChain = layer0Chain.find(chain => chain.chainName === compatibleChains[0]);
      setDestinationChainInfo(firstCompatibleChain);
    } else {
      setDestinationChainInfo(null);  // If no compatible chains, reset the destination chain
    }

    const incompatibleChains = layer0Chain
      .map(chain => chain.chainName)
      .filter(chainName => !compatibleChains.includes(chainName));

    setIncompatibleChains(incompatibleChains);
    setIsOpenSrcChain(false);
  };



  const handleDestinationChainChange = (event) => {
    const selectedChain = layer0Chain.find(chain => chain.chainName === event.target.dataset.value);

    // Check if the selected destination chain is the same as the current source chain
    if (selectedChain.chainId === srcChainInfo.chainId) {
      // Swap source and destination chains
      const oldSrcChainInfo = srcChainInfo;
      setSrcChainInfo(destinationChainInfo);
      setDestinationChainInfo(oldSrcChainInfo);
    } else {
      setDestinationChainInfo(selectedChain);
    }

    setIsOpenDistChain(false);
  };
  ...
  // SMART CONTRACT INTEGRATION INTO MINT AND BRIDGE BUTTON

  // SMART CONTRACT INITIALIZATION

  // ENDPOINT

  const contractEndpoint = new ethers.Contract(
    srcChainInfo.endpoint,
    endpointAbi,
    signer
  )
  //OUR CONTRACT
  const starLabContract = new ethers.Contract(
    srcChainInfo.contractAddress,
    abi,
    signer
  )
  ...
  const mintNewNFT = async () => {
    ...
    try {
      setIsMinting(true);
      const mintCostPerChain = ethers.utils.parseEther(srcChainInfo.mintCost);
      const tx = await starLabContract.mint({ value: mintCostPerChain });
      await tx.wait();
      let tokenId;
      ...
      for (let i = 0; i < receipt.logs.length; i++) {
        try {
          const parsedLog = contractInterface.parseLog(receipt.logs[i]);
          if (parsedLog.name === 'Transfer') {
            tokenId = parsedLog.args.tokenId.toString();
            break;
          }
        } catch (error) {
          console.error(`Failed to parse log index ${i}: `, error);
        }
      }
      setIsMinting(false);

      toast.success(`Successfully minted NFT #${tokenId}`);
      setTokenId(tokenId);
    } catch (error) {
      setIsMinting(false);
  ...
  const bridgeNFT = async () => {
    try {
      setIsBridging(true);
      const gasEstimateDistChain = await contractEndpoint.estimateFees(
        destinationChainInfo.chainId,
        srcChainInfo.contractAddress,
        '0x',
        false,
        '0x0001000000000000000000000000000000000000000000000000000000000005cc60'
      );
      const gasEstimationLayer0 = gasEstimateDistChain.nativeFee.toString();
      toast(`Optimized gas fee for destination chain : ${parseFloat(ethers.utils.formatEther(gasEstimationLayer0)).toFixed(4)} ${srcChainInfo.currency} `);

      const bridgeTx = await starLabContract.crossChain(destinationChainInfo.chainId, tokenId, { value: gasEstimationLayer0 });

      await bridgeTx.wait();
      setIsBridging(false);

      const bridgeHash = bridgeTx.hash;
      toast.success(`Successfully Bridged into ${destinationChainInfo.chainDisplay}!`);


      if (bridgeHash !== 0) {
        const link = `https://layerzeroscan.com/tx/${bridgeHash}`;
        setTxHash(link);
      }
      if (!showInput) {
        toggleInput();
      }
      toast.success('You can now bridge your NFT to another chain !');
    }
    catch (error) {
      setIsBridging(false);
      ...
      if (error.message.includes('invalid BigNumber string')) {
        const customMessage = 'Mint an NFT first or insert your NFT ID above';
  ...
  useEffect(() => {
    ...
    const checkNetwork = async () => {
      try {
        // Only proceed if an account is connected
        // Get the current network ID
        const networkId = await window.ethereum.request({ method: 'eth_chainId' });

        // Get the desired chain ID from the chains.json file
        const desiredChainId = chains[srcChainInfo.chainName].chainId;

        // Compare the current network ID with the source chain's network ID
  ...
  }, [srcChainInfo, isAccountConnected]);


  const [isOpenSrcChain, setIsOpenSrcChain] = useState(false);
  const [isOpenDistChain, setIsOpenDistChain] = useState(false);

  const handleToggleSrcChain = () => {
    setIsOpenSrcChain(!isOpenSrcChain);
    setIsOpenDistChain(false); // Close the destination chain dropdown
  };

  const handleToggleDistChain = () => {
    setIsOpenDistChain(!isOpenDistChain);
    setIsOpenSrcChain(false); // Close the source chain dropdown
  };
  ...
  return (
    <>
      <BridgePageContainer>
        <BridgeSquare>
          <BridgeTitle>Star Lab Bridge</BridgeTitle>
          <DropdownContainer>
            <div>
              {/* Source chain dropdown */}
              <DropdownList className={`dropdown-btn ${isOpenSrcChain ? 'open' : ''}`}>
                <div className='chainContainer' onClick={handleToggleSrcChain}>

                  <img className='chainLogo' src={srcChainInfo.image} />
                  <div>{srcChainInfo.chainDisplay}</div>
                </div>
                <img onClick={handleToggleSrcChain} src={arrow} className='arrowLogo' />


              </DropdownList>
              <div className={`dropdown-content ${isOpenSrcChain ? 'show' : ''}`}>
                {/* <div className='dropdown-item chainContainerdropdown' data-value='zkSyncEraTestnet' onClick={handleSourceChainChange}>zkSync Era </div>
              <div className='dropdown-item chainContainerdropdown' data-value='Goerli' onClick={handleSourceChainChange}>Goerli ETH</div>
              <div className='dropdown-item chainContainerdropdown' data-value='optimism' onClick={handleSourceChainChange}>Optimism</div> */}
                {layer0Chain.map(chain => (
                  <div onClick={handleSourceChainChange} data-value={chain.chainName} className={`chainContainerdropdown`}>
                    <img src={chain.image} data-value={chain.chainName} className='chainImage' />
                    <div className='dropdown-item' data-value={chain.chainName}  >
                      {chain.chainDisplay}
                    </div>
                  </div>
                ))}
            ...
            <SwapButton onClick={swapChains}>
              <SwapImage src={swapImage} alt="Swap" />
            </SwapButton>
            <div>
              {/* Destination chain dropdown */}
              <DropdownList className={`dropdown-btn ${isOpenDistChain ? 'open' : ''}`}>



                <div className='chainContainer' onClick={handleToggleDistChain}  >

                  <img className='chainLogo' src={destinationChainInfo.image} />
                  <div>{destinationChainInfo.chainDisplay}</div>
                </div>

                <img onClick={handleToggleDistChain} src={arrow} className='arrowLogo' />
              </DropdownList>
              <div className={`dropdown-content ${isOpenDistChain ? 'show' : ''}`}>
                {[...layer0Chain]
                  .sort((a, b) => (incompatibleChains.includes(a.chainName) ? 1 : -1))
                  .map(chain => (
                    <div
                      onClick={handleDestinationChainChange}
                      data-value={chain.chainName}
                      className={`chainContainerdropdown ${incompatibleChains.includes(chain.chainName) ? 'blur-out' : ''}`}
                    >
                      <img src={chain.image} data-value={chain.chainName} className='chainImage' />
                      <div className='dropdown-item' data-value={chain.chainName}>
                        {chain.chainDisplay}
          ...
          <StepTitle>Get your NFT</StepTitle>
          {isOnRightNetwork ? (

            account ? (
              <MintButton onClick={mintNewNFT} disabled={isMinting} >  {isMinting ? "Mint ongoing..." : "Mint"}</MintButton>
            ) : <ConnectFirst>Connect your wallet first !</ConnectFirst>


          )
            : <ConnectFirst>YOU'RE ON THE WRONG NETWORK</ConnectFirst>
          }
          ...
          <StepContainer>
            <Step2Title>Bridge it to your desired chain !</Step2Title>
            <SettingsButton onClick={toggleInput}>
              <SettingsImage src={settingsImage} alt="Settings" />
            </SettingsButton>
          </StepContainer>
          ...
          <InputContainer showInput={showInput}>
            <InputLabel htmlFor="mintId">NFT ID to be bridged:</InputLabel>
            <Input id="mintId" type="text" placeholder="Enter mint ID" value={tokenId} onChange={e => setTokenId(e.target.value)} />
          </InputContainer>

          {isOnRightNetwork ? (
            account ? (
              <BridgeButton onClick={bridgeNFT} disabled={isBridging} > {isBridging ? "Bridge ongoing..." : "Bridge"}
              </BridgeButton>
            ) : <ConnectFirst>Connect yeour wallet first !</ConnectFirst>
          ) : <ConnectFirst>YOU'RE ON THE WRONG NETWORK</ConnectFirst>
