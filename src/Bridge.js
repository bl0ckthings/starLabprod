...
const DropdownLabel = styled.label`
  font-size: 16px;
`;
// const SourceChain = styled.label`
//   font-size: 16px;
// `;
...
const BridgePage = ({ ethereumProvider, switchChain, account }) => {
  ...
  const defaultDestinationChain = layer0Chain.find(chain => chain.chainName === 'optimism');
  ...
  useEffect(() => {
    // Get the compatible chains for the default source chain
    const compatibleChains = compatibilityMap[defaultSrcChain.chainName] || [];
  ...
  const handleSourceChainChange = (event) => {
    ...
    // Get the compatible chains for the selected source chain
    const compatibleChains = compatibilityMap[selectedChainName] || [];

    // If there is at least one compatible chain, set it as the destination chain
      ...
      for (let i = 0; i < receipt.logs.length; i++) {
        try {
          ...
          if (parsedLog.name === 'Transfer') {
            ...
            break; // Exit the loop once we found and logged the tokenId
          }
        } catch (error) {
  ...
  const bridgeNFT = async () => {
    try {
      ...
      toast.success(`Optimized gas fee for destination chain : ${parseFloat(ethers.utils.formatEther(gasEstimationLayer0)).toFixed(4)} ETH `);
          ...
          <DropdownContainer>
            ...
            <div>
              ...
              <div className={`dropdown-content ${isOpenDistChain ? 'show' : ''}`}>
                {layer0Chain.map(chain => (
                  <div onClick={handleDestinationChainChange} data-value={chain.chainName} className={`chainContainerdropdown ${incompatibleChains.includes(chain.chainName) ? 'blur-out' : ''}`}>
                    <img src={chain.image} data-value={chain.chainName} className='chainImage' />
                    <div className='dropdown-item' data-value={chain.chainName}>
                      {chain.chainDisplay}
